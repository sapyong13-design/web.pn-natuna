...
<article class="ampuh-directory" data-ampuh-directory>
  ...
  <section class="ampuh-directory__tools" aria-label="Pencarian direktori"><label for="ampuh-directory-search">Cari dokumen AMPUH</label><input id="ampuh-directory-search" type="search" data-ampuh-search autocomplete="off" placeholder="Cari GOBI, checklist, atau nama dokumen"><div data-ampuh-gobi-filter aria-label="Filter GOBI"><?php foreach ($directory['gobis'] as $gobi) : ?><button type="button" data-ampuh-filter-value="<?php echo $escape((string) (int) $gobi['number']); ?>"><?php echo $escape($gobiLabel($gobi)); ?></button><?php endforeach; ?></div><button type="button" data-ampuh-close-all>Tutup semua</button><p data-ampuh-results aria-live="polite"></p></section>
  <div class="ampuh-directory__tree">
    <?php foreach ($directory['gobis'] as $gobi) : $gobiId = 'ampuh-gobi-' . (int) $gobi['number']; $gobiDisplay = $gobiLabel($gobi); $gobiText = $normalized($gobiDisplay . ' ' . (string) $gobi['name']); ?>
      <section class="ampuh-directory__gobi" data-search-text="<?php echo $escape($gobiText); ?>" data-ampuh-gobi="<?php echo (int) $gobi['number']; ?>"><h2><button type="button" data-ampuh-toggle aria-expanded="false" aria-controls="<?php echo $gobiId; ?>"><?php echo $escape($gobiDisplay); ?></button></h2><div id="<?php echo $gobiId; ?>" data-ampuh-panel hidden>

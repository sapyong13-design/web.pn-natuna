...
<article class="ampuh-directory" data-ampuh-directory>
  <header class="ampuh-directory__header">
    <div class="ampuh-directory__hero-main"><p class="ampuh-directory__kicker">Direktori Bukti</p><h1><?php echo $escape((string) ($directory['title'] ?? 'AMPUH 2026 Checklist')); ?></h1><p class="ampuh-directory__lede"><?php echo $escape((string) ($directory['summary'] ?? '')); ?></p><?php $driveAction($directory['main_drive_url'], 'Buka Folder Utama AMPUH 2026'); ?></div>
  ...
  <section class="ampuh-directory__tools" aria-label="Pencarian direktori"><div class="ampuh-directory__search"><label for="ampuh-directory-search">Cari dokumen AMPUH</label><input id="ampuh-directory-search" type="search" data-ampuh-search autocomplete="off" placeholder="Cari GOBI, checklist, atau nama dokumen"></div><button type="button" data-ampuh-close-all>Tutup semua</button><div class="ampuh-directory__filter-rail" data-ampuh-gobi-filter aria-label="Filter GOBI"><?php foreach ($directory['gobis'] as $gobi) : ?><button type="button" data-ampuh-filter-value="<?php echo $escape((string) (int) $gobi['number']); ?>" aria-pressed="false"><?php echo $escape($gobiLabel($gobi)); ?></button><?php endforeach; ?></div><div class="ampuh-directory__gobi-select"><label for="ampuh-gobi-select">Pilih GOBI</label><select id="ampuh-gobi-select" data-ampuh-gobi-select><option value="">Semua GOBI</option><?php foreach ($directory['gobis'] as $gobi) : ?><option value="<?php echo $escape((string) (int) $gobi['number']); ?>"><?php echo $escape($gobiLabel($gobi)); ?></option><?php endforeach; ?></select></div><p data-ampuh-results aria-live="polite"></p></section>
        ...
        <?php foreach ($gobi['checklists'] as $checklist) : $checklistId = $gobiId . '-checklist-' . (int) $checklist['number']; $checklistText = $normalized((string) $checklist['title']); $checklistSubchecklistCount = count($checklist['subchecklists']); $checklistFileCount = 0; foreach ($checklist['subchecklists'] as $countSubchecklist) { $checklistFileCount += count($countSubchecklist['files']); } ?>
          <section class="ampuh-directory__checklist" data-search-text="<?php echo $escape($checklistText); ?>"><h3><button type="button" data-ampuh-toggle aria-expanded="false" aria-controls="<?php echo $checklistId; ?>"><?php echo $escape((string) $checklist['title']); ?> <span class="ampuh-directory__meta"><?php echo $checklistSubchecklistCount; ?> sub-checklist · <?php echo $checklistFileCount; ?> dokumen</span></button></h3><?php $driveAction((string) ($checklist['drive_url'] ?? ''), 'Buka folder checklist'); ?><div id="<?php echo $checklistId; ?>" data-ampuh-panel hidden>
            <?php foreach ($checklist['subchecklists'] as $subchecklist) : $subId = $checklistId . '-sub-' . preg_replace('/[^a-z0-9]+/i', '-', (string) $subchecklist['number']); $filesId = $subId . '-files'; $subText = $normalized((string) $subchecklist['title']); ?>
              <section class="ampuh-directory__subchecklist" data-ampuh-result data-search-text="<?php echo $escape($subText); ?>"><h4><button type="button" data-ampuh-toggle aria-expanded="false" aria-controls="<?php echo $escape($subId); ?>"><span class="ampuh-directory__sub-number"><?php echo $escape((string) $subchecklist['number']); ?></span><span class="ampuh-directory__sub-title"><?php echo $escape((string) preg_replace('/^\s*' . preg_quote((string) $subchecklist['number'], '/') . '\s*[.:-]?\s*/u', '', (string) $subchecklist['title'])); ?></span></button></h4><?php $driveAction((string) ($subchecklist['drive_url'] ?? ''), 'Buka folder sub-checklist'); ?><div id="<?php echo $escape($subId); ?>" data-ampuh-panel hidden><h5><button type="button" data-ampuh-toggle aria-expanded="false" aria-controls="<?php echo $escape($filesId); ?>">Daftar dokumen (<?php echo count($subchecklist['files']); ?>)</button></h5><div id="<?php echo $escape($filesId); ?>" data-ampuh-panel hidden><ul class="ampuh-directory__files"><?php foreach ($subchecklist['files'] as $file) : ?><li data-ampuh-result data-search-text="<?php echo $escape($normalized((string) $file)); ?>"><span class="ampuh-directory__file-icon" aria-hidden="true"><?php echo $fileType((string) $file); ?></span><?php echo $escape((string) $file); ?></li><?php endforeach; ?></ul></div></div></section>
            <?php endforeach; ?>
          </div></section>
        <?php endforeach; ?>
      </div></section>

...
- Hero beranda **(25 Jul 2026: rotasi dibongkar)**: satu komposisi statis, bukan lagi slider. Sebelumnya tiga slide bergantian tiap 7 detik sehingga dua dari tiga komposisi selalu tersembunyi — termasuk daftar berita, yang merupakan satu-satunya tempat berita pengadilan sendiri muncul di beranda (`Pengumuman & Video Terbaru` hanya pengumuman; `Kabar Instansi Peradilan` milik MA/Badilum/PT Kepri). Sekarang grid dua kolom `.hero-stack-grid`: kiri sambutan (status PTSP, headline, intro, ribbon tiga data, dua CTA), kanan `.hero-news-card` (tiga berita kategori 12: satu lead berfoto + dua ringkas) dan `.hero-integrity` (pita Zona Integritas). Backdrop tetap pre-graded `images/hero/gedung-pn-natuna-2026-graded.webp` dengan `fetchpriority=high`, feather mask 11%, dan animasi zoom yang dipause selama scroll.
- Poster **Tolak Gratifikasi & Pungutan Liar** turun jadi lampiran, bukan slide: pita ZI menaut ke `/zona-integritas`, dan pratinjau `…-480.webp` membuka poster penuh (1672×941, 211KB) lewat lightbox `[data-maklumat-zoom]` yang sudah dipakai kartu Maklumat. Dihapus bersama slider: `.hero-slide*`, `.hero-slider-dots`, `.hero-nav`, `.hero-tab*`, `setupHeroNewsTabs()`, dan init carousel `.hero-slider` — 119 aturan CSS mati (−731 baris). Kontrak: `tools/test_integrity_hero_slide.php` (kini menjaga agar rotasi tidak kembali).
- Warna teks hero wajib dinyatakan pada bloknya sendiri. `.hero h2 { color: var(--color-primary-deep) }` dan `.hero p { color: #34414c }` bersifat global; dulu ditahan oleh `.home-slider .hero-slide:not(.hero-slide-news)` yang ikut terhapus. Tanpa deklarasi eksplisit, headline dan intro berubah gelap di atas foto. Kolom kiri juga punya scrim lokal `.hero-cinema .hero-welcome-copy::before` supaya headline tetap ≥3:1 saat melewati atap yang terang; menggelapkan scrim backdrop global akan mengubur gedung.

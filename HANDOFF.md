...
- Hero beranda **(25 Jul 2026: rotasi dibongkar, lalu kartu melayang dibuang)**: satu komposisi statis, bukan slider. Sebelumnya tiga slide bergantian tiap 7 detik sehingga dua dari tiga komposisi selalu tersembunyi — termasuk daftar berita, satu-satunya tempat berita pengadilan sendiri muncul di beranda (`Pengumuman & Video Terbaru` hanya pengumuman; `Kabar Instansi Peradilan` milik MA/Badilum/PT Kepri).
- Susunan hero adalah **tiga lapis melebar**, bukan kartu di atas foto: `.hero-stage` (panggung foto, sambutan di kiri: status PTSP, headline, intro, lalu `.hero-facts` yang mensejajarkan ribbon tiga data dengan dua CTA dalam satu baris), `.hero-newsbar` (tiga berita kategori 12, tiap item bergambar), dan `.hero-pledge` (Zona Integritas satu baris). Dua yang terakhir dibungkus `.hero-footbar` — alas gelap yang melebar sampai tepi hero lewat margin negatif `--page-padding`. **Percobaan pertama menempelkan dua kartu bersudut-bulat ke paruh kanan foto dan itu ditolak:** dua bahasa desain bertabrakan, gedung tertutup scrim di kiri dan kartu di kanan, dan pratinjau poster 92px tidak terbaca. Jangan mengembalikannya; `tools/test_integrity_hero_slide.php` menolak `.hero-news-card`, `.hero-integrity`, `.hero-aside`, dan `.hero-stack-grid`.
- `.hero-facts` menyejajarkan ribbon dan tombol. Menumpuknya memakan ~85px dan mendorong pita berita ke bawah lipatan; sebaris membuat seluruh hero muat tanpa scroll di 1920×1080 dan 1440×900. Di 1366×768 pita masih ~205px di bawah lipatan — penyebabnya header 295px, bukan hero.
- Poster **Tolak Gratifikasi & Pungutan Liar** tidak lagi dimuat di hero dalam bentuk apa pun. `.hero-pledge` hanya teks; tombol **Lihat poster** membuka berkas penuh (1672×941, 211KB) lewat lightbox `[data-maklumat-zoom]` milik kartu Maklumat. Dihapus bersama slider: `.hero-slide*`, `.hero-slider-dots`, `.hero-nav`, `.hero-tab*`, `setupHeroNewsTabs()`, dan init carousel `.hero-slider` — 119 aturan CSS mati (−731 baris).
- Warna teks hero wajib dinyatakan pada bloknya sendiri. `.hero h2 { color: var(--color-primary-deep) }` dan `.hero p { color: #34414c }` bersifat global; dulu ditahan oleh `.home-slider .hero-slide:not(.hero-slide-news)` yang ikut terhapus. Tanpa deklarasi eksplisit, headline dan intro berubah gelap di atas foto. Kolom kiri punya scrim lokal `.hero-cinema .hero-welcome-copy::before`; menggelapkan scrim backdrop global akan mengubur gedung. Kredit foto pindah ke kanan-atas dan pil latarnya dikembalikan lewat `.hero-cinema .hero-stage .hero-photo-chip` — `.hero-cinema .hero-photo-chip` membuatnya transparan, aman di atas scrim kiri tapi menjatuhkan kontras ke 1,23 di atas langit. Terukur setelah perbaikan: headline 4,30, intro 8,76, judul berita 17,11, baris ZI 18,73.
...
- **Stabilitas layout 25 Jul 2026 — CLS 1,109 → 0,011 (desktop 1366).** Tiga sumber geseran, semuanya "kotak tidak dicadangkan sebelum JS mengisi":
  ...
  Verifikasi: CLS 0,0055 (1920), 0,0113 (1366), 0,0241 (1024), 0,0050 (390) — semua di bawah ambang 0,1, tanpa kliping dan overflow horizontal 0.

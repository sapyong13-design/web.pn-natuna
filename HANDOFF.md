...
- Pengumuman & Video Terbaru: homepage memakai komposisi desktop 45:55 berisi satu pengumuman kategori **13** dan maksimal lima video channel resmi `UCuPb35OggK2PKdW7Ed0qszA`. Dua video wajib selalu di awal: `-Di2t-yUZ1I` (**Video Profile Pengadilan Negeri / Perikanan Ranai**) dan `kQ0dMRp1W_g` (**Tata cara penggunaan e-Berpadu**), lalu tiga video Atom terbaru unik. Data dibaca dari `cache/pn_natuna_youtube/feed.json`; `tools/cron-refresh-youtube.php` memperbarui cache per jam dan mempertahankan cache valid saat feed gagal. Renderer berada di `hero-slider.php`, domain/cache di `youtube-feed.php`, dan iframe `youtube-nocookie.com` baru dibuat setelah klik Putar oleh `template.js`. Kontrak: `tools/test-youtube-feed.php`, `tools/test_latest_announcements_showcase.php`, dan `tools/test_youtube_showcase.php`.
- Polish 18 Jul 2026: card pengumuman tidak lagi dipaksa setinggi panel video; preview dokumen memakai crop kepala dokumen dan copy rapat. Panel video memakai label editorial, konteks channel, tombol play lingkaran, tiga navigator terbaca dengan dua item lanjutan via horizontal scroll, serta status `Sedang dipilih`. Mobile menempatkan video sebelum pengumuman, memberi peek rail, dan menjaga overflow section nol.

...
def check(cid,hidden,stable=None):
 ...
 # Judul milik migrasi lain: 109 menjadi "Kontak" sejak IA navigasi mobile
 # 20260722, dan 108 bisa ikut berubah. Yang wajib dijaga di sini adalah URUTAN
 # AMPUH tepat setelah Transparansi; judul AMPUH sendiri sudah dicek di atas.
 if mysql("SELECT GROUP_CONCAT(id ORDER BY lft) FROM pnn_menu WHERE menutype='mainmenu' AND parent_id=1 AND id IN (108,109,"+cid+")",DATABASE)!=f'108,{cid},109': raise RuntimeError('navbar order failed')
 ...
 if mysql("SELECT COUNT(*) FROM pnn_menu WHERE alias='cross-type-child'",DATABASE)!='0': raise RuntimeError('cross-menutype duplicate child remains')
...
def main():
 ...
 try:
  ...
  duplicate=mysql("SELECT id FROM pnn_menu WHERE title='Duplicate AMPUH'",DATABASE)
  mysql("INSERT INTO pnn_menu (menutype,title,alias,note,path,link,type,published,parent_id,level,component_id,checked_out,checked_out_time,browserNav,access,img,template_style_id,params,lft,rgt,home,language,client_id) SELECT 'hidden','Cross type duplicate child','cross-type-child','','cross-type-child','index.php?option=com_content&view=article&id=1','component',1,"+duplicate+",2,extension_id,NULL,NULL,0,1,'',0,'{}',0,0,0,'*',0 FROM pnn_extensions WHERE element='com_content' AND type='component' LIMIT 1; INSERT INTO pnn_modules_menu (moduleid,menuid) VALUES (900001,"+duplicate+"),(900002,-"+duplicate+");",DATABASE)
  parent=1
